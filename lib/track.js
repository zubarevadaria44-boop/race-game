...
function buildTrack(def) {
  ...
  const rocketPickups = [0.22, 0.48, 0.74].map((fraction, idx) => {
    const index = Math.floor(centerline.length * fraction);
    const [x, y] = centerline[index];
    return { id: `rocket-${idx}`, x, y };
  });

  const boostPads = [0.35, 0.62].map((fraction, idx) => {
    const index = Math.floor(centerline.length * fraction);
    const [x, y] = centerline[index];
    return { id: `boost-${idx}`, x, y };
  });
...
export function drawPowerups(ctx, track, pickupState, now) {
  ...
  track.rocketPickups.forEach((pickup) => {
    ...
    if (state && !state.active) {
      const remaining = Math.max(0, state.respawnAt - now);
      if (remaining > 0) {
